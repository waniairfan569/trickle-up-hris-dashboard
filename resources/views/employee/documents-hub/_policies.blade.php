...
<div class="space-y-5" x-data="{ tab: 'all' }">
    <div class="flex items-center gap-1 border-b border-slate-200 dark:border-slate-700">
        ...
        <button @click="tab = 'pending'" :class="tab === 'pending' ? 'border-brand-500 text-brand-600 dark:text-brand-400' : 'border-transparent text-slate-400 hover:text-slate-600'" class="border-b-2 px-4 py-2.5 text-sm font-bold transition">To do @if($policyPendingCount)<span class="ml-1 inline-flex items-center justify-center rounded-full bg-amber-100 text-amber-700 px-1.5 text-[10px]">{{ $policyPendingCount }}</span>@endif</button>
    ...
    <div class="space-y-3">
        ...
        @empty
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm py-16 text-center dark:bg-slate-800 dark:border-slate-700">
                ...
                <p class="text-sm font-bold text-slate-600 dark:text-slate-300">No policies pending acknowledgment</p>
                <p class="text-xs text-slate-400 mt-1">You're up to date.</p>
            </div>
        @endforelse
    </div>
</div>
