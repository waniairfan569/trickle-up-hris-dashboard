{{-- "All" tab — the company Document Library (browse / download / acknowledge). --}}
<div class="space-y-5" x-data="{ search: '', lcat: 'all' }">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1">
            <button @click="lcat='all'" :class="lcat==='all' ? 'bg-brand-600 text-slate-900' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300'" class="shrink-0 rounded-full px-4 py-1.5 text-sm font-bold">All</button>
            @foreach($categories as $category)
                <button @click="lcat='{{ $category->id }}'" :class="lcat==='{{ $category->id }}' ? 'bg-brand-600 text-slate-900' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300'" class="shrink-0 inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-sm font-bold"><i data-lucide="{{ $category->icon ?: 'folder' }}" class="h-3.5 w-3.5"></i> {{ $category->name }}</button>
            @endforeach
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($documents as $doc)
                ...
                <div data-name="{{ Str::lower($doc->title.' '.$doc->description) }}" data-cat="{{ $doc->category_id }}"
                     x-show="(lcat==='all' || lcat==='{{ $doc->category_id }}') && (search==='' || $el.dataset.name.includes(search.toLowerCase()))"
                    ...
                    @if($doc->requires_acknowledgment)
                        @php $ack = $docAcks[$doc->id] ?? null; @endphp
                        @if($ack)
