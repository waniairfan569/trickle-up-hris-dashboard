...
<div class="space-y-5" x-data="{ tab: 'all' }">
    ...
    @if(!empty($periods))
        <div class="flex flex-wrap items-center gap-1.5">
            ...
            @endphp
            <a href="{{ route('documents-hub.index', ['tab' => 'forms']) }}" class="{{ $pill(!$selectedPeriod || $selectedPeriod === 'all') }}">All</a>
            @foreach($periods as $p)
                <a href="{{ route('documents-hub.index', ['tab' => 'forms', 'period' => $p]) }}" class="{{ $pill($selectedPeriod === $p) }}">{{ \App\Models\CompanyForm::periodLabel($p) }}</a>
            @endforeach
        </div>
    @endif
    ...
    <div class="space-y-3">
        ...
        @empty
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm py-16 text-center dark:bg-slate-800 dark:border-slate-700">
                ...
                <p class="text-sm font-bold text-slate-600 dark:text-slate-300">No forms assigned to you right now.</p>
