...
<body class="bg-slate-50 text-slate-900 antialiased font-sans flex items-center justify-center min-h-screen">
    <div class="max-w-md w-full p-6">
        <div class="text-center mb-8">
            ...
            <p class="text-slate-500 mt-2">Account Setup</p>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-6">
                <h2 class="text-xl font-bold text-slate-800 mb-2">Set your new password</h2>
                
                @if (session('warning'))
                    <div class="mb-4 bg-amber-50 text-amber-800 p-4 rounded-lg text-sm flex items-start">
                ...
                @else
                    <p class="text-slate-500 text-sm mb-6">You must set a new password before you can access the system.</p>
                @endif
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input id="current_password" type="password" name="current_password" required autofocus
                                class="w-full rounded-lg border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 @error('current_password') border-red-300 focus:border-red-500 focus:ring-red-500 @enderror">
            ...
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 text-center">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-sm text-brand-600 hover:text-brand-800 font-medium">Log out instead</button>
                </form>
            </div>
        </div>
    </div>
