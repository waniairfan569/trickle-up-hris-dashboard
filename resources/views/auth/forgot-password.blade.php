...
<head>
    ...
    <title>Reset your password · Trickle Up HRIS</title>
...
<body class="h-full font-sans text-slate-100 flex flex-col justify-center items-center overflow-x-hidden relative bg-slate-950 px-6">
    ...
    <div class="w-full max-w-md space-y-8 z-10">
        <div class="flex flex-col items-center text-center space-y-4">
            ...
            <h2 class="text-2xl font-extrabold text-white tracking-tight">Forgot your password?</h2>
            <p class="text-xs text-slate-400 max-w-xs leading-relaxed">Enter the email tied to your account and we'll send you a secure link to set a new password.</p>
        </div>

        <div class="glow-effect p-8 rounded-3xl bg-slate-900/40 border border-slate-800 backdrop-blur-xl space-y-6">
            @if(session('status'))
                <div class="rounded-2xl bg-emerald-500/10 border border-emerald-500/20 p-4 flex gap-3">
                    <i data-lucide="mail-check" class="h-5 w-5 text-emerald-400 flex-shrink-0"></i>
                    <div class="text-xs font-semibold text-emerald-300">{{ session('status') }}</div>
                </div>
            @endif
            @if($errors->any())
                <div class="rounded-2xl bg-rose-500/10 border border-rose-500/20 p-4 flex">
                    <i data-lucide="alert-circle" class="h-5 w-5 text-rose-400 flex-shrink-0"></i>
                    <div class="ml-3 text-xs font-semibold text-rose-300">{{ $errors->first() }}</div>
                </div>
            @endif
