{{-- Shared "Day at a glance": date bar + Celebrations / Announcements / Upcoming
     events cards + out-of-office bar. Used by the employee and admin dashboards.
     Needs $celebrations, $events, $holidays, $announcements, $outOfOffice,
     $workFromHome, $remoteWorkers (all provided by DashboardController). --}}
@php
    $quotes = [
    ...
    $dagQuote = $quotes[now()->dayOfYear % count($quotes)];
@endphp
<div class="space-y-4" x-data="celebrationsWidget()">
    ...
    <div class="relative flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/70 bg-white px-5 py-4 shadow-sm dark:bg-slate-800 dark:border-slate-700">
        <div class="flex items-center gap-3 text-slate-700 font-semibold dark:text-slate-200 relative" @click.away="showPicker = false">
            ...
            <div>
                ...
                <p class="flex items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400"><i data-lucide="sun" class="h-3.5 w-3.5 text-amber-500"></i> {{ $dagQuote }}</p>
            </div>

            <!-- Custom Calendar Dropdown -->
            <div x-show="showPicker" x-transition x-cloak
                ...
                <div class="grid grid-cols-7 gap-1">
                    <template x-for="d in getPickerDays()" :key="d.dateString">
                        <button type="button" @click.stop="selectPickerDate(d.dateString)"
                                class="h-7 w-7 rounded-lg flex items-center justify-center text-[10px] transition"
                                :class="[d.dateString === current ? 'bg-blue-600 text-white font-bold' : (d.currentMonth ? 'text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 font-medium' : 'text-slate-300 dark:text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800')]"
                                x-text="d.day"></button>
                    </template>
                </div>
            </div>
    ...
    function celebrationsWidget() {
        return {
            ...
            initPicker() { const d = new Date(this.current + 'T00:00:00'); this.pickerYear = d.getFullYear(); this.pickerMonth = d.getMonth(); },
            getPickerDays() {
                ...
                for (let i = firstDayIndex - 1; i >= 0; i--) {
                    ...
                    days.push({ day: d, currentMonth: false, dateString: `${yStr}-${mStr}-${String(d).padStart(2, '0')}` });
                }
                for (let i = 1; i <= totalDays; i++) {
                    const mStr = String(month + 1).padStart(2, '0');
                    days.push({ day: i, currentMonth: true, dateString: `${year}-${mStr}-${String(i).padStart(2, '0')}` });
                }
                const remaining = 42 - days.length;
                for (let i = 1; i <= remaining; i++) {
                    const mStr = String(month === 11 ? 1 : month + 2).padStart(2, '0');
                    const yStr = month === 11 ? year + 1 : year;
                    days.push({ day: i, currentMonth: false, dateString: `${yStr}-${mStr}-${String(i).padStart(2, '0')}` });
                }
                return days;
            },
            selectPickerDate(dateStr) { this.current = dateStr; this.showPicker = false; },
            changePickerMonth(dir) {
                this.pickerMonth += dir;
                if (this.pickerMonth > 11) { this.pickerMonth = 0; this.pickerYear++; }
                else if (this.pickerMonth < 0) { this.pickerMonth = 11; this.pickerYear--; }
            },
