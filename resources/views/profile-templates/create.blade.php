...
@section('breadcrumb', 'Create Profile Template')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-x-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Create Profile Template</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Design a new layout structure for specific employee profiles.</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm dark:bg-slate-800 dark:border-slate-850">
        <form action="{{ route('profile-templates.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g., Engineer Profile Template, Contractor Form" required
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="4" placeholder="Briefly describe what department, role, or region this profile layout serves..."
                    class="block w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-950 placeholder-slate-400 focus:border-brand-500 focus:ring-brand-500 dark:border-slate-700 dark:bg-slate-850 dark:text-white focus:outline-none focus:ring-2 focus:ring-offset-0 transition duration-150">{{ old('description') }}</textarea>
            </div>
