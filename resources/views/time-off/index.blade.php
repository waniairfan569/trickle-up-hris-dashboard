    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        @forelse($timeOffBalances as $balance)
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 dark:bg-slate-800 dark:border-slate-700/80">
                ...
                @if($isWfh)
                    @php
                        $wfhReq = \App\Models\TimeOffRequest::where('user_id', auth()->id())->where('policy_id', $balance->policy_id)->whereYear('start_date', now()->year);
                    ...
                    @endphp
                    <div class="text-xs font-bold text-brand-600 dark:text-brand-400 text-right mb-4">Approved per request</div>
