            ...
            <div x-show="tab === 'all' || tab === '{{ $form->status }}'" class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5 dark:bg-slate-800 dark:border-slate-700">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-slate-400 mt-1">{{ $form->fields_count }} fields · {{ $form->submitted_count }}/{{ $form->assigned_count }} submitted @if($form->deadline) · due {{ $form->deadline->format('d M Y') }} @endif</p>
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <form action="{{ route('company-forms.destroy', $form) }}" method="POST" onsubmit="return confirm('Delete “{{ $form->title }}”?');">@csrf @method('DELETE')<button type="submit" class="inline-flex h-7 w-7 items-center justify-center rounded-lg text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10"><i data-lucide="trash-2" class="h-4 w-4"></i></button></form>
                    </div>
                </div>
            </div>
        @empty
