    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden dark:bg-slate-800 dark:border-slate-700">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                @php $hasFields = $rows->contains(fn ($r) => $r['ack'] && !empty($r['ack']->field_values)); @endphp
                <thead><tr class="text-left text-[11px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100 dark:border-slate-700/60">
                    ...
                    @if($hasFields)<th class="px-5 py-3">Submitted details</th>@endif
                </tr></thead>
                <tbody>
                    @forelse($rows as $r)
                        <tr class="border-b border-slate-100 last:border-0 dark:border-slate-700/60">
                            ...
                            @if($hasFields)
                                <td class="px-5 py-2.5">
                                    @if($r['ack'] && !empty($r['ack']->field_values))
                                        <div class="flex flex-wrap gap-1.5">
                                            @foreach($r['ack']->field_values as $token => $value)
                                                <span class="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5 text-[11px] dark:bg-slate-700">
                    ...
                    @empty
                        <tr><td colspan="{{ $hasFields ? 5 : 4 }}" class="px-5 py-12 text-center text-sm text-slate-400">No eligible employees for this document.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
