...
@section('breadcrumb', 'Edit Policy')
...
<div class="max-w-4xl mx-auto space-y-6" x-data="policyForm()">
    <div class="sm:flex sm:items-center sm:justify-between border-b border-slate-200/80 pb-5 dark:border-slate-700/60">
        <div>
            <h2 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Edit Policy: <span class="text-brand-600">{{ $timeOffPolicy->name }}</span></h2>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Update the rules for this policy.
            </p>
        </div>
    ...
    <form action="{{ route('time-off-policies.update', $timeOffPolicy) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-1">
                        ...
                        <input type="text" name="name" value="{{ old('name', $timeOffPolicy->name) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm dark:bg-slate-900 dark:border-slate-600 dark:text-white">
                    </div>
                    <div class="md:col-span-1">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="2" class="w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm dark:bg-slate-900 dark:border-slate-600 dark:text-white">{{ old('description', $timeOffPolicy->description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p x-show="accrualType === 'monthly'" class="mt-2 text-xs font-bold text-brand-600 dark:text-brand-400" x-text="`≈ ${Math.round((daysPerYear / 12) * 10) / 10} days accrued per month`"></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden dark:bg-slate-800 dark:border-slate-700/80">
            ...
            <div class="p-8" x-show="carryOver" style="display: none;">
                <div class="w-full md:w-1/2">
                    ...
                    <input type="number" step="0.5" name="carry_over_max" value="{{ old('carry_over_max', $timeOffPolicy->carry_over_max) }}" placeholder="Leave blank for unlimited" class="w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm dark:bg-slate-900 dark:border-slate-600 dark:text-white">
                </div>
            </div>
        </div>
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="pb-6 border-b border-slate-100 dark:border-slate-700/60">
                    ...
                    <div x-show="requiresApproval" class="pl-8 pt-2">
                        ...
                        <select name="approval_type" class="w-full md:w-1/2 rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm dark:bg-slate-900 dark:border-slate-600 dark:text-white">
                            <option value="manager" {{ old('approval_type', $timeOffPolicy->approval_type) == 'manager' ? 'selected' : '' }}>Direct Manager Only</option>
                            <option value="hr_admin" {{ old('approval_type', $timeOffPolicy->approval_type) == 'hr_admin' ? 'selected' : '' }}>HR Admin Only</option>
                            <option value="super_admin" {{ old('approval_type', $timeOffPolicy->approval_type) == 'super_admin' ? 'selected' : '' }}>Super Admin Only</option>
                            <option value="both" {{ old('approval_type', $timeOffPolicy->approval_type) == 'both' ? 'selected' : '' }}>Manager then HR Admin</option>
                            <option value="manager_super" {{ old('approval_type', $timeOffPolicy->approval_type) == 'manager_super' ? 'selected' : '' }}>Manager then Super Admin</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                    <div>
                        ...
                        <input type="number" name="min_notice_days" value="{{ old('min_notice_days', $timeOffPolicy->min_notice_days) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm dark:bg-slate-900 dark:border-slate-600 dark:text-white">
                    </div>
                </div>

                <div class="pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="allow_half_days" value="1" {{ old('allow_half_days', $timeOffPolicy->allow_half_days) ? 'checked' : '' }} class="rounded border-slate-300 text-brand-600 shadow-sm focus:ring-brand-500 dark:border-slate-600 dark:bg-slate-900">
                        <span class="ml-2 text-sm font-bold text-slate-700 dark:text-slate-300">Allow Half-Day Requests</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" name="allow_negative_balance" value="1" {{ old('allow_negative_balance', $timeOffPolicy->allow_negative_balance) ? 'checked' : '' }} class="rounded border-slate-300 text-brand-600 shadow-sm focus:ring-brand-500 dark:border-slate-600 dark:bg-slate-900">
                        <span class="ml-2 text-sm font-bold text-slate-700 dark:text-slate-300">Allow Negative Balances</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_paid" value="1" {{ old('is_paid', $timeOffPolicy->is_paid) ? 'checked' : '' }} class="rounded border-slate-300 text-emerald-600 shadow-sm focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-900 dark:checked:bg-emerald-500">
                        <span class="ml-2 text-sm font-bold text-slate-700 dark:text-slate-300">This is Paid Leave</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $timeOffPolicy->is_active) ? 'checked' : '' }} class="rounded border-slate-300 text-emerald-600 shadow-sm focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-900 dark:checked:bg-emerald-500">
    ...
    <form id="delete-form" action="{{ route('time-off-policies.destroy', $timeOffPolicy) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
</div>
...
<script>
    function policyForm() {
        return {
            daysPerYear: {{ old('days_per_year', $timeOffPolicy->days_per_year) }},
            accrualType: '{{ old('accrual_type', $timeOffPolicy->accrual_type) }}',
            carryOver: {{ old('carry_over', $timeOffPolicy->carry_over) ? 'true' : 'false' }},
            requiresApproval: {{ old('requires_approval', $timeOffPolicy->requires_approval) ? 'true' : 'false' }}
        }
    }
</script>
@endsection
