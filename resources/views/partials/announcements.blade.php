    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 dark:bg-slate-800 dark:border-slate-700"
        ...
        <div class="flex items-center justify-between gap-3 mb-4">
            <div class="flex items-center gap-3">
                <div class="h-11 w-11 rounded-2xl bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center text-brand-500 dark:text-brand-400">
        ...
        <div class="space-y-3">
            @foreach($announcements as $a)
                @php
                    $preview = \Illuminate\Support\Str::limit(trim(preg_replace('/\s+/', ' ', strip_tags($a->bodyHtml()))), 160);
                    $meta = (optional($a->creator)->full_name ?? 'Admin') . ' · ' . $a->created_at->diffForHumans();
                @endphp
                <div class="flex flex-col rounded-2xl border p-5 transition hover:shadow-sm {{ $a->is_pinned ? 'border-brand-200 bg-brand-50/40 dark:border-brand-500/20 dark:bg-brand-500/5' : 'border-slate-200/70 bg-white dark:border-slate-700/60 dark:bg-slate-900/30' }}">
                    <div class="flex items-start gap-2">
                        @if($a->is_pinned)<i data-lucide="pin" class="h-3.5 w-3.5 text-brand-500 mt-1 shrink-0"></i>@endif
                    ...
                    <div class="flex items-center justify-between gap-2 mt-4">
                        <span class="text-[11px] text-slate-400 truncate">{{ $meta }}</span>
                        <button type="button"
                                @click="cur = @js(['title' => $a->title, 'body' => (string) $a->bodyHtml(), 'meta' => $meta, 'pinned' => (bool) $a->is_pinned]); open = true"
                                class="inline-flex items-center gap-1.5 rounded-full bg-white border border-slate-200 px-3 py-1.5 text-[11px] font-bold text-slate-700 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-200 shrink-0">
