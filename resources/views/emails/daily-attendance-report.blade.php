@php
    $company = config('app.name', 'Trickle Up');
    $s = $data['summary'];
    $isTeam = $recipient && method_exists($recipient, 'isManager') && $recipient->isManager() && !$recipient->isAdmin();
    $rowColor = [
        'present' => '#ffffff', 'late' => '#FFF3CD', 'absent' => '#FFE5E5',
        'on_leave' => '#E6F1FB', 'overtime' => '#E8F5E9', 'early_departure' => '#FFF3E0',
        'missing_clock_out' => '#F3E5F5',
    ];
@endphp
<!DOCTYPE html>
<html>
...
<table width="640" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
    ...
    <!-- Header -->
    <tr><td style="background:#1B2D5E;padding:28px 32px;color:#ffffff;">
        <div style="font-size:13px;letter-spacing:1px;text-transform:uppercase;color:#aab6d6;font-weight:bold;">{{ $company }} HRIS</div>
        <div style="font-size:24px;font-weight:bold;margin-top:6px;">Daily Attendance Report</div>
        <div style="font-size:14px;color:#cdd7ee;margin-top:4px;">{{ $data['date']->format('l, d M Y') }}</div>
        @if($isTeam)
            <div style="margin-top:10px;display:inline-block;background:rgba(255,255,255,.12);padding:5px 12px;border-radius:999px;font-size:12px;font-weight:bold;">Your Team Report — {{ $recipient->full_name }}</div>
        @endif
    </td></tr>
    ...
    <tr><td style="padding:24px 32px 8px;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr>
            ...
            <td width="25%" style="padding:4px;">
                <div style="background:#FFF3CD;border-radius:10px;padding:{{ $s['late'] > 0 ? '18px 14px' : '14px' }};text-align:center;{{ $s['late'] > 0 ? 'border:2px solid #f0ad4e;' : '' }}">
                    <div style="font-size:11px;color:#8a6d3b;font-weight:bold;text-transform:uppercase;">⚠️ Late</div>
                    <div style="font-size:{{ $s['late'] > 0 ? '30px' : '26px' }};font-weight:bold;color:#8a6d3b;">{{ $s['late'] }}</div>
                </div>
            </td>
            <td width="25%" style="padding:4px;">
                <div style="background:#FFE5E5;border-radius:10px;padding:14px;text-align:center;">
            ...
            <td width="25%" style="padding:4px;">
                <div style="background:#E6F1FB;border-radius:10px;padding:14px;text-align:center;">
                    <div style="font-size:11px;color:#1565c0;font-weight:bold;text-transform:uppercase;">🏖️ On Leave</div>
    ...
    <tr><td style="padding:16px 32px;">
        <div style="font-size:16px;font-weight:bold;border-left:4px solid #f0ad4e;padding-left:10px;margin-bottom:10px;">Late Arrivals Today</div>
        @if(count($data['late_employees']) > 0)
            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:13px;">
                <tr style="background:#fbe9c6;text-align:left;">
                ...
                <tr style="background:#FFF3CD;border-top:1px solid #f3e2b3;">
        ...
        @else
            <div style="background:#E8F5E9;color:#1b5e20;border-radius:8px;padding:12px;text-align:center;font-weight:bold;">🎉 No late arrivals today!</div>
        @endif
    </td></tr>
    @endif
    ...
    <tr><td style="padding:8px 32px 16px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:13px;">
            ...
            <tr style="background:#FFE5E5;border-top:1px solid #f3c0c0;">
                <td style="padding:8px 10px;font-weight:bold;">{{ $e['name'] }}</td>
                <td style="padding:8px 10px;color:#64748b;">{{ $e['department'] ?? '—' }}</td>
            </tr>
            @endforeach
    ...
    <tr><td style="padding:8px 32px 24px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:12px;">
            <tr style="background:#1B2D5E;color:#fff;text-align:left;">
    ...
    <tr><td style="background:#f8fafc;padding:20px 32px;border-top:1px solid #e2e8f0;font-size:11px;color:#94a3b8;text-align:center;">
        This is an automated report generated by {{ $company }} HRIS.<br>
        Report generated at {{ now()->format('d M Y, h:i A') }}.<br>
        <a href="{{ route('attendance-reports.settings') }}" style="color:#1565c0;">Change report settings</a><br>
