...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 bg-slate-50">
            <h2 class="text-xl font-bold text-slate-800">Edit Office Location: {{ $officeLocation->name }}</h2>
        </div>
        
        <form action="{{ route('office-locations.update', $officeLocation) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" required class="w-full border-slate-300 rounded-lg shadow-sm focus:border-brand-500 focus:ring-brand-500" value="{{ old('name', $officeLocation->name) }}">
                        @error('name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="address" class="block text-sm font-medium text-slate-700 mb-1">Address</label>
                        <textarea name="address" id="address" rows="2" class="w-full border-slate-300 rounded-lg shadow-sm focus:border-brand-500 focus:ring-brand-500">{{ old('address', $officeLocation->address) }}</textarea>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="latitude" class="block text-sm font-medium text-slate-700 mb-1">Latitude *</label>
                            <input type="number" step="0.0000001" name="latitude" id="latitude" required class="w-full border-slate-300 rounded-lg shadow-sm focus:border-brand-500 focus:ring-brand-500" value="{{ old('latitude', $officeLocation->latitude) }}">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.0000001" name="longitude" id="longitude" required class="w-full border-slate-300 rounded-lg shadow-sm focus:border-brand-500 focus:ring-brand-500" value="{{ old('longitude', $officeLocation->longitude) }}">
                    ...
                    <div>
                        ...
                        <input type="number" name="radius_meters" id="radius_meters" required min="50" max="5000" class="w-full border-slate-300 rounded-lg shadow-sm focus:border-brand-500 focus:ring-brand-500" value="{{ old('radius_meters', $officeLocation->radius_meters) }}">
                    ...
                    <div class="pt-4 border-t border-slate-200">
                        <label class="flex items-center">
                            <input type="checkbox" name="allow_remote" id="allow_remote" value="1" class="rounded border-slate-300 text-brand-600 shadow-sm focus:border-brand-300 focus:ring focus:ring-brand-200 focus:ring-opacity-50" {{ old('allow_remote', $officeLocation->allow_remote) ? 'checked' : '' }}>
            ...
            <div class="mt-8 flex justify-end space-x-3">
                ...
                <button type="submit" class="bg-brand-600 hover:bg-brand-700 text-white font-bold py-2 px-6 rounded-lg shadow-sm transition">Update Location</button>
            </div>
        </form>
    </div>
</div>
